...
<h1 class="mb-6 text-xl font-semibold">Estadísticas de mi biblioteca</h1>

{#if data.stats.total === 0}
	<p class="text-neutral-400">
		Todavía no hay datos suficientes. Agrega títulos desde <a
			href="/buscar"
			class="text-blue-400 hover:underline">Buscar</a
...
{:else}
	<div class="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
		<div class="rounded-lg bg-neutral-800 p-4 text-center">
			...
			<p class="text-xs text-neutral-400">Títulos en total</p>
		</div>
		<div class="rounded-lg bg-neutral-800 p-4 text-center">
			<p class="text-2xl font-bold">
				{data.stats.averageRating ? data.stats.averageRating.toFixed(2) : '—'}
			</p>
			<p class="text-xs text-neutral-400">Rating promedio</p>
		</div>
	</div>

	<div class="grid gap-8 md:grid-cols-2">
		<section>
			<h2 class="mb-3 text-sm font-semibold text-neutral-300">Distribución por estado</h2>
			<div class="flex flex-col gap-2">
				{#each statusRows as row (row.label)}
					<div class="flex items-center gap-3 text-sm">
						<span class="w-24 shrink-0 text-neutral-400">{row.label}</span>
						<div class="h-3 flex-1 rounded bg-neutral-800">
							<div
								class="h-3 rounded bg-blue-600"
								style="width: {(row.count / maxStatus) * 100}%"
							></div>
						</div>
						<span class="w-6 shrink-0 text-right text-neutral-400">{row.count}</span>
					</div>
				{/each}
			</div>
		</section>
		...
		<section>
			<h2 class="mb-3 text-sm font-semibold text-neutral-300">Películas vs. series</h2>
			<div class="flex flex-col gap-2">
				{#each mediaTypeRows as row (row.label)}
					<div class="flex items-center gap-3 text-sm">
						...
						<div class="h-3 flex-1 rounded bg-neutral-800">
							<div
								class="h-3 rounded bg-purple-600"
		...
		<section class="md:col-span-2">
			<h2 class="mb-3 text-sm font-semibold text-neutral-300">Géneros más frecuentes</h2>
			...
			{:else}
				<div class="flex flex-col gap-2">
					{#each data.stats.topGenres as genre (genre.genre)}
						<div class="flex items-center gap-3 text-sm">
							<span class="w-32 shrink-0 truncate text-neutral-400">{genre.genre}</span>
							<div class="h-3 flex-1 rounded bg-neutral-800">
								<div
							...
							<span class="w-6 shrink-0 text-right text-neutral-400">{genre.count}</span>
						</div>
					{/each}
				</div>
			{/if}
